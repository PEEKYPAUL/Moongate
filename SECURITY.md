...
- **Status polling only reads.** Each poll reads Klipper state through Moonraker: `print_stats`, `extruder`, `heater_bed`, `display_status`, `virtual_sdcard`, `webhooks` (Klipper's ready/shutdown state), the discovered chamber sensor, the extra hotends and tool changer on multi-tool printers, a configured light's status object, the printing file's metadata, and - for the printer shown on the Single-printer dashboard only (v0.9.69) - the toolhead position and homed axes. No other Klipper objects, and nothing is written.
- **The plugin does not run arbitrary G-code on behalf of token holders.** There is no `POST /server/moongate/gcode` endpoint. Print control is restricted to the whitelisted actions above plus one addition in plugin 0.6.16: **`update_plugin`**, which triggers Moonraker's *own* update manager for the Moongate component (the app's one-tap plugin update). It updates exactly one thing - the already-installed Moongate component, from its already-configured source - is refused while a print is running or paused, and gives a token holder no new capability they didn't have via Mainsail's Software Update panel. Adding further actions requires editing [`klipper-plugin/moongate_standalone.py`](klipper-plugin/moongate_standalone.py) and auditing the new behaviour.
- **Temperature watches (plugin 0.6.27+).** `/server/moongate/temp-watch` (GET / POST / DELETE) is auth-gated exactly like `/control`. A token holder can arm a watch that sets heater targets (`SET_HEATER_TEMPERATURE` for a named heater, targets outside 0-400 °C are ignored), waits for temperatures, and optionally starts a stored G-code file by path once the watch fires - and only if the printer is idle at that moment. That is the same reach a token holder already has through Moonraker's own `print/start` and `gcode/script` routes behind the tunnel; the plugin adds no new capability and still runs no arbitrary G-code. The `MOONGATE_TEMP_NOTIFY` macro arms the same kind of watch from the printer's own config, needing no token. Watches are stored as `temp_watches.json` in the plugin's data directory.
- **The plugin reads its own state from `~/.config/moongate/`** and calls `systemctl` / `journalctl` to discover the current tunnel URL when its own log lookup fails. It does not read `printer.cfg` or any other Klipper internals beyond what Moonraker exposes. The one file it writes in Moonraker's config directory is the installer-managed `moongate.cfg` (0.6.27+): at start it appends any Moongate macro missing from that file so badge-updated printers get the macros a re-run of the installer would have written; a `moongate.cfg` without the installer's header is left alone, and `printer.cfg` is never touched.

The auth proxy is even more constrained - it doesn't talk to Klipper at all, it just routes HTTP / WebSocket between Cloudflare and nginx. The verifier classes it uses for token checks are imported from the plugin file, not duplicated, so signature semantics stay single-source. Since plugin **0.6.14** it also keeps **no per-request access log**: earlier versions wrote one to `/run/moongate-authproxy.log` on the Pi, including the short-lived `mg_token` query values (local to the Pi and cleared on reboot, but more than a proxy needs to record); now only its own warnings and errors are emitted, into the systemd journal. Plugin **0.6.18** adds exactly one class of line to that: when the external-camera relay **fails** to deliver a frame (rejected target, camera error, or an empty answer), one short journal line records the outcome - with the camera address stripped to scheme, host and path, so neither `mg_token` nor any camera credential can appear. Successful frames still log nothing.
...
**In-app reports.** The `feedback` table is locked down like the rest of the schema - no client read/write. Only the `submit-feedback` Edge Function (service role) writes; only the Supabase dashboard or the secret-gated `read-feedback` function reads (it requires an `x-moongate-debug` header matched against a server-only `MOONGATE_DEBUG_KEY`, so the public anon key can't read it). Reports carry diagnostics - app / device / network / per-printer connection state, and (newer app versions) each printer's webcam fetch state with camera addresses stripped of their query strings - but no print content, files, tokens, or personal data.

**Released printers & plugin dormancy (v0.9.49-v0.9.50).** Removing a printer now **soft-deletes** its record at the middleman instead of erasing it at once; the existing cleanup job hard-deletes it after one week. Two consequences worth stating. *Data retention:* for up to a week after you remove a printer, the middleman still holds its record (name, device public key, last-seen time - never print content); before v0.9.49 that data disappeared immediately. This is deliberate: the tombstone is what lets the printer's own heartbeat receive an explicit **"released"** answer, so the Pi plugin (0.6.15+) stops contacting the middleman right away instead of retrying a dead record forever. *Enumeration:* "released" vs "never existed" is only ever revealed to a caller whose request is **signed by that printer's own device key** - the signature is verified before any lookup - so nothing about other users' printers can be probed. The dormant plugin keeps a 6-hourly pulse and wakes instantly on `MOONGATE_PAIR`, so recovery needs no special steps.
...
## Table grants are explicit (September 2026)

Supabase no longer grants Data API access to new tables in the `public` schema automatically (new projects since 30 May 2026, existing projects from 30 October 2026). Moongate's tables all predate that and kept their grants, so the live service was never affected: a new user pairing a printer only ever creates *rows* in tables that already exist. What did change is the schema source. Every earlier migration relied on the automatic grants and only ever `REVOKE`d, so a rebuild from [`supabase/migrations/`](supabase/migrations/) after the cut-over would have produced tables nobody could reach. Migration `20260923120000_explicit_table_grants.sql` now states the whole posture in one place, and it is narrower than the old defaults: `service_role` (the Edge Functions) holds `SELECT/INSERT/UPDATE/DELETE` on the five tables and nothing more; `authenticated` (the app's anonymous session) holds `SELECT` on `printers` only, filtered by the "select own printers" policy; the bare `anon` role holds nothing; and the `TRUNCATE`, `REFERENCES` and `TRIGGER` privileges the old defaults handed out are revoked from every role. The Pi never touches the Data API at all, it only calls Edge Functions. Verify query and the rule for future tables: [`supabase/README.md`](supabase/README.md) section 3.5.
...
| What about per-printer access tokens? | [`mobile/lib/services/printer_access_cache.dart`](mobile/lib/services/printer_access_cache.dart) - short-lived (30 min TTL), in-memory only, refreshed via the middleman |
