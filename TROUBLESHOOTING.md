...
## The printer page turned into GitHub, Spoolman or the docs, and nothing brings Mainsail back

Mainsail and Fluidd link out all over the place - a commit hash in the Update Manager opens GitHub, the AFC panel links to Spoolman, the sidebar links to the docs - and on a desktop those open in a new browser tab. Up to v0.9.69 the printer page in Moongate followed the link instead, and because Moongate keeps each printer's page alive for instant re-opening, the printer *became* that page: Back returned to the dashboard, opening the printer again showed GitHub, and nothing in Moongate led back. From **v0.9.70** a link to anything other than the printer's own address opens in an in-app browser tab (Android) or a Safari view (iPhone) with its own Close, and the printer page stays put; a page that has still ended up elsewhere is put back on the printer when you open it. Away from home (over the tunnel) a link into the printer's own network, such as Spoolman on the Pi, shows a note that it can't be reached from there: the tunnel only carries the printer page itself, not the other services on the Pi. On an older build, the **refresh arrow** in the printer page's top bar reloads the printer from scratch, which brings Mainsail back.
...
From v0.9.52 the app understands **go2rtc** cameras (a `stream.html?src=...` address in Mainsail's webcam settings): the dashboard tile and the full-screen camera show them as live stills automatically, whether auto-detected from Mainsail or pasted into a tile's gear. Away from home a go2rtc camera follows the external-camera rules above: the address in Mainsail's webcam settings must be a numeric LAN `http://` URL (like `http://192.168.1.20:1984/stream.html?src=cam`), or the tile shows only the logo remotely.
...
## A USB webcam through go2rtc: "Connecting…" everywhere after a reboot, or the picture crawls at 1-3 fps

Two go2rtc-with-a-USB-webcam problems look like app faults but live on the Pi. Both show the same way from **every** viewer at once - the app's tiles, Mainsail's camera panel on a PC, and a plain `curl http://<pi>:1984/api/frame.jpeg?src=NAME` all wait forever or crawl - and that is the tell that the app is not the cause.

- **Everything says "Connecting to …" after the Pi rebooted, and go2rtc's log says `device or resource busy`.** The kernel put the idle camera into USB autosuspend a few seconds after boot and the wake-up failed: `dmesg` shows `uvcvideo … reset_resume error -13` right after a `reset high-speed USB device` line. From then on the camera answers nobody until it is re-plugged. Recover without a reboot: stop go2rtc, re-enumerate just the camera's port - `echo 0 | sudo tee /sys/bus/usb/devices/<port>/authorized; sleep 2; echo 1 | sudo tee /sys/bus/usb/devices/<port>/authorized` (the port is the `usb 1-1.x` name next to the camera in `dmesg`; leave the printer's own USB devices alone) - and start go2rtc again. Stop it recurring with a udev rule that keeps the camera awake, in `/etc/udev/rules.d/`: `ACTION=="add", SUBSYSTEM=="usb", ATTR{idVendor}=="046d", ATTR{idProduct}=="082d", TEST=="power/control", ATTR{power/control}="on"` (your camera's vendor and product ids come from `lsusb`).
- **The picture crawls at 1-3 fps once the room is dark, and is fine in daylight.** Many UVC cameras (the Logitech C920 included) ship with "dynamic frame rate" on: in low light they slow the sensor right down to gather light, and every viewer gets those few frames. `v4l2-ctl -d /dev/video0 -c exposure_dynamic_framerate=0` restores the full rate immediately, at the price of a slightly darker, noisier picture with the lights off (older kernels call the control `exposure_auto_priority`). The setting resets whenever the camera re-enumerates, so put it in the same udev rule or in the go2rtc service's `ExecStartPre`. Set camera controls **one per `v4l2-ctl` call** - a C920 refuses `focus_absolute` in the same call that turns autofocus off, and the whole call fails.

To tell the two apart, ask go2rtc itself: `curl -s "http://<pi>:1984/api/streams?src=NAME"` twice a few seconds apart. The producer's `packets` counter is the camera's real frame rate; each consumer's `packets` and `drops` are what that viewer received and lost. A producer stuck on the same number is the first problem, one climbing by about 3 a second is the second, and a producer climbing by 30 while the consumers drop most of it means the network between the Pi and that viewer is the limit, not the camera.
