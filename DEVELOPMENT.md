...
The plugin reaches Moonraker as a **symlink**, `~/moonraker/moonraker/components/moongate.py` → `klipper-plugin/moongate_standalone.py`, so a `git pull` of the clone is the update. Moonraker's update manager runs `git status` in its own checkout and would flag that link as an untracked source file in the Software Update panel, so the link's path goes on Moonraker's per-clone ignore list, `.git/info/exclude` (never committed, survives Moonraker's own updates and its hard-recovery `git clean`): `install.sh` writes it at install time and, since plugin 0.6.28, the plugin writes it again at every component load; `uninstall.sh` removes it.

> **Moonraker never executes `update.sh`.** The `install_script` option the plugin is registered under in `[update_manager moongate]` is deprecated and only scanned for `PKGLIST` lines (Moonraker's docs: "Moonraker will not run this script"), so a panel or badge update is a `git pull` plus a Moonraker restart and nothing more. Found 25/09/2026: every migration `update.sh` carried since 0.6.14 had only ever run where someone re-ran the installer or invoked it by hand. Hence `_post_update_chores()` in `moongate_standalone.py`: the chores that need no root (the exclude line, the `moongate-tunnel` entry in `moonraker.asvc` that the tunnel watchdog depends on, a fresh copy of `moongate-pair.html` in the web roots, and the macro blocks via `_refresh_macros_cfg()`) run inside the plugin at load, idempotently, on every update path. `update.sh` stays the manual tool for the one chore that needs root, the pre-0.6.14 authproxy journal migration. Anything new that must follow an update goes in the plugin, not in the script. See [TROUBLESHOOTING.md](TROUBLESHOOTING.md#software-update-panel-says-moonraker-has-untracked-source-files-moongatepy).
...
> **The Supabase backend is not part of CI.** Database migrations under [`supabase/migrations/`](supabase/migrations/) and the Edge Functions under [`supabase/functions/`](supabase/functions/) are deployed **manually** against the live project - `supabase db push` for migrations and `supabase functions deploy <name>` for functions - never by GitHub Actions. So a PR that adds a migration (e.g. `20260619120000_printers_realtime.sql`, which enabled Realtime on the `printers` table for v0.9.16) or changes a function (e.g. the access-token TTL in `_shared/accessToken.ts`) only takes effect once it's pushed to Supabase by hand. Full backend setup / deploy flow: [`supabase/README.md`](supabase/README.md). **A migration that creates a table must carry its `GRANT`s** (plus RLS and policies, as one unit): Supabase stopped granting Data API access to new `public` tables automatically on 30 October 2026, and a table without an explicit grant is unreachable by the Edge Functions and the app. `20260923120000_explicit_table_grants.sql` is the model; the verify query is in README section 3.5.
