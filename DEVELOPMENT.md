...
`ci.yml` runs `flutter analyze` and `flutter test` on PRs (unit tests live in `mobile/test/`; the Pi plugin additionally has standalone stdlib-only tests under `klipper-plugin/tests/` - dependency isolation and the heartbeat pairing-window behaviour - each runnable with plain `python3` on any machine).
