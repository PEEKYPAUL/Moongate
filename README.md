...
- 🛠️ **Console & config editor** (v0.9.61+) - two buttons under every online printer's temperatures. **Console** opens the printer's live G-code console as an overlay: recent history already filled in, live updates while open, errors in red, quick-command chips (they fill the input, never send by themselves), and tap any past command to reuse it. It stays available mid-print and even while Klipper is in an error state - exactly when a console matters. **File System** browses the printer's configuration folder and opens `printer.cfg` (and friends) in a **structured editor**: setting names stay fixed as labels, you edit just the values, and your comments and layout survive a save untouched. A backup copy is saved on the printer before your first change, **Save and restart** brings Klipper up on the new config and watches it come back - and if it doesn't, one tap restores the backup. The browser keeps itself tidy too: a **Hide backups & hidden files** checkbox (on by default, v0.9.63+) filters Klipper's SAVE_CONFIG snapshots, `.bak` files and dotfiles out of the list until you untick it - the choice is remembered and travels in backups. Works on Wi-Fi and over the tunnel alike, even while Klipper itself is down, with no plugin update needed.
