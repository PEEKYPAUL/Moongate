...
└─ GoRouter
    ...
    ├─ /pair         → PairingScreen
    ├─ /printer/:id  → PrinterScreen (WebView, kept warm by PrinterWebViewCache; a link off the printer's own origin opens outside it - webview_link_policy.dart, v0.9.70)
...
| `heatsoak_timers.dart` + `models/heat_alerts.dart` | The preheat sheet's **heat-soak alert** (v0.9.67). The sheet arms one record per printer - the temperatures it just set (hotend(s), bed, and a chamber value to wait for on printers that report a chamber sensor) plus a soak time - and the print-notification service's background isolate judges it against the live readings on every poll: a heater counts as at temperature within 3 °C of its *current* target, the chamber against the value typed (the sheet insists on a bed temperature alongside it, since the bed is what heats the chamber), heaters switched off cancel the wait, and a soak time starts a clock only once everything is reached (the moment is written back so a service restart keeps counting). Fires the one-shot **Heat alerts** notification ("At temperature" or "Heat-soak complete") then clears itself. The rules are pure Dart in `models/heat_alerts.dart` (unit-tested); the arms live in local preferences and never ride a backup |
| `models/temp_watch.dart` | **Temperature watches** (v0.9.68, plugin 0.6.27+): the printer-side cousin of the heat-soak alert. The Start-print dialog arms an `app-soak` watch (preheat the bed / chamber heater, wait, soak, then the plugin starts the print itself if still idle) and an `app-cool` watch ("ready to remove": after the print has run and ended, bed + chamber back within 5 °C of the readings at Start) through `PrintControlService.armTempWatch` → `POST /server/moongate/temp-watch`; alert texts are worded on the phone with the plugin's `{bed}` / `{chamber}` / `{mins}` placeholders so they arrive localised. `/status` reports the live watches (`temp_watches`), which `PrinterStatus.tempWatches` carries to the tile's one-line "Soaking · 12 min left · then file" (tap = cancel via `DELETE`). Pure Dart, unit-tested |
...
| `webview_link_policy.dart` | **Where a link inside the printer WebView goes** (v0.9.70). Mainsail / Fluidd never navigate the main frame themselves, so a main-frame navigation request is the user tapping a link. The printer's own origin - same host and port as the loaded LAN / Direct / tunnel address, or the same host switching http ↔ https on the standard ports - stays in the WebView; anything else (a GitHub commit in the Update Manager, the AFC panel's Spoolman link on port 7912, go2rtc on 1984, the docs, `mailto:`) opens outside in an in-app browser tab (Android Custom Tab / iOS Safari view via `url_launcher`'s `inAppBrowserView`, falling back to the external app). Over the tunnel (`_usingLan` false) a link into a private / local network (`linkIsOnPrinterNetwork`: RFC 1918, link-local, loopback, IPv6 ULA / link-local, `localhost`, `.local` / `.lan` / bare names) gets a snack bar instead, since the tunnel never carried anything but the printer page. `PrinterScreen`'s navigation delegate applies it and answers `prevent`; a re-attached warm session whose current URL is off-origin is loaded back onto the printer page. Before this the kept-warm WebView followed every link and the printer "became" GitHub with no way back. Pure function, unit-tested (`webview_link_policy_test.dart`). |
...
| `webcam_fetch_diag.dart` | In-memory, per-printer record of how the webcam fetch loop is doing - which camera URL is in play (query strings stripped, so never a token), the last fetch result, and how many attempts have failed in a row. Read by the in-app bug report, so a camera that never produces a frame - previously invisible in every diagnostic - is now self-explanatory in support |
| `print_progress.dart` | Pure helper that computes print progress as a 0-1 fraction matching Mainsail's default *file position (relative)* mode - the printed byte position mapped onto the slicer's gcode body. Shared by the status service and the notification so the tile, the notification, and Mainsail all show the same number (v0.9.17). As of v0.9.53 it also holds the shared **time-remaining / finish-clock estimate** (elapsed ÷ progress, gated until ~2% in), used by both the tile's time chip and the notification card |
...
- **Tunnel watchdog (0.6.23+).** The one gap in the self-healing story used to be the tunnel itself: heartbeats retry forever and `cloudflared` reconnects after a normal network blip, but a tunnel whose long-lived connection is silently dropped for good (an ISP flap, a router that quietly killed it) would just sit there - the printer reads offline remotely while everything on the Pi looks healthy, and the person it strands is by definition away from home. The watchdog closes that gap. Right after a heartbeat **succeeds** (which proves the internet is up at that moment), the plugin fetches its **own** public tunnel URL with a deliberately-bogus token and expects the auth proxy's `401`. Only a transport failure or Cloudflare's own tunnel-down answer (HTTP 530) counts against it - any delivered response, even a 502, proves the tunnel path works. Three strikes (~15 minutes) and it restarts `moongate-tunnel` through Moonraker's **allowed-services** machine API - the same mechanism the web UIs' service buttons use, so no new `sudo`; the installer adds `moongate-tunnel` to `moonraker.asvc`, and since plugin 0.6.28 the plugin re-adds it itself at every load, because an install updated only through the Software Update panel had never received it (see the post-update chores bullet below) - which mints a fresh URL that the very next heartbeat reports. Guard-railed: at most one probe every 4 minutes, a heal budget of 3 per rolling 6 hours before it stands down with one loud log line (a tunnel that stays dead through fresh URLs needs a human), and it never acts during a WAN outage (heartbeats fail too, so no probe runs and `cloudflared` reconnects on its own once the connection returns). Off with `[moongate] tunnel_watchdog: False`; inert in LAN-only mode. **Zero cloud cost** - probes hit the printer's own tunnel, not the middleman.
- **Status.** Aggregates Klipper / Moonraker objects into a single `/server/moongate/status` response - `print_stats`, temperatures, the discovered chamber sensor, webcam transforms, the current tunnel URL, and (0.6.4+) the plugin's own version, which the app compares against the version it shipped with to drive the dashboard's **plugin-update badge**. Since **0.6.23** it also carries a small `tunnel_watchdog` block (heal count, last heal, state), so the app's diagnostics can tell whether a printer's tunnel has been quietly self-healing. Since **0.6.26** it carries `last_notify` - the last accepted `MOONGATE_NOTIFY` message as `{seq, ts, text}`, recorded before the push is attempted - which is how those messages reach Android: iPhones get a push, Android has no push path, so the app's notification service polls `/status` and alerts when the sequence number changes while it is watching (a fresh plugin reports `null`, which the app treats as sequence 0 so the first message after a restart still alerts; the very first sighting after the service starts is a silent baseline, so nothing replays).
...
- **Temperature watches (0.6.27+).** `/server/moongate/temp-watch` (GET / POST / DELETE, owner-token gated like `/status`) and the `MOONGATE_TEMP_NOTIFY` macro (`moongate_temp_notify` remote method) arm one watch per source (`macro`, `app-soak`, `app-cool`): targets by role (extruder / extruderN / bed / chamber), a direction per target fixed from the reading at arm time (`up` = warm to within 3 °C of the LIVE target, a passive chamber against the typed value; `down` = at or below), optional soak minutes (clock starts at temperature, a sag does not reset it), optional `then_start` (Moonraker `print/start` when it fires, only if `print_stats` is idle) and `after_print` (judging waits until a print has run and ended; `cooldown_delta` builds the goals from the readings now). Judged by a 20 s asyncio loop that only runs while a watch exists, persisted to `<data_dir>/temp_watches.json` so a Moonraker restart keeps counting, 6 h stale / 1 h late drops as in the app's rules, fired once through the same path as `MOONGATE_NOTIFY` (push `custom` event + `last_notify` + an `M118` echo). At start the plugin also appends any Moongate macro missing from an installer-managed `moongate.cfg` (`macros_pending_restart` in `/status`) - the installer only ever wrote that file once, so badge-updated printers had been missing `MOONGATE_STATUS` / `MOONGATE_NOTIFY`.
- **`MOONGATE_STATUS` console macro (0.6.23+).** A support-first diagnostic run from the Klipper console. It performs one **live signed heartbeat** - the cloud-database half, authoritative, and it also freshens `last_seen` - and the watchdog's own read-only tunnel probe (the tunnel half), then prints a plain-language verdict for each with the fix named in the same breath: re-pair for a released or missing registration, a clock sync for a skew-refused heartbeat (the classic "works on LAN, never remote"), the right journal for a dead auth proxy. All wording lives in two pure functions, so the whole matrix is unit-tested with no network. Installed alongside `MOONGATE_PAIR` / `MOONGATE_RESET_OWNER`; since 0.6.27 the plugin appends it to an installer-managed `moongate.cfg` at load, and Klipper picks it up on its next restart.
...
- **Dependency-free LAN-only + embedded hosts (0.6.17+).** The cloud crypto libraries (PyJWT, cryptography) load only when cloud mode uses them, and LAN-only builds **none** of the cloud machinery (no device-key generation, no key file) - so LAN-only needs **zero Python packages beyond Moonraker's own**, which is what lets Moongate run on embedded vendor firmware with no pip (see [3rd-party printer support](docs/third-party-printers.md)). Three optional `[moongate]` options in `moonraker.conf` - `lan_only`, `data_path` (relocates all plugin state off `~/.config` for read-only-home systems), `moonraker_port` - override `config.json`, and the plugin now discovers Moonraker's own port from the server (embedded builds bind `:80`, not `:7125`). A cloud-mode install whose crypto libraries are missing still loads, keeps the LAN endpoints alive, and reports the exact problem in the log, the `MOONGATE_PAIR` console output, and its error responses.
- **Post-update chores (0.6.28+).** Moonraker's update manager never executes an extension's `install_script` (the option is deprecated and only scanned for a `PKGLIST` line naming apt packages), so an update from the Software Update panel or the app's badge is a `git pull` plus a Moonraker restart and nothing more. Everything `klipper-plugin/update.sh` was meant to do after a pull had therefore only ever run where someone re-ran the installer or invoked it by hand (found 25/09/2026). Component load is the one hook every update path shares, so the plugin now does the chores that need no root there, idempotently and best effort: its own path on Moonraker's per-clone ignore list (`.git/info/exclude`, which stops the panel reporting an untracked source file), the `moongate-tunnel` line in `moonraker.asvc` plus an in-session allow so the watchdog works from that load rather than the next restart, a fresh copy of the QR pairing page in the web roots, and (since 0.6.27) the macro blocks in an installer-managed `moongate.cfg`. The one chore that needs root, the pre-0.6.14 auth-proxy log migration, stays in `update.sh` for a manual run.
...
`cloudflared` Quick Tunnel runs as `moongate-tunnel.service`. It makes an outbound connection to the Cloudflare edge - no inbound ports opened on your router. The URL rotates each time the Pi reboots; the Moongate plugin heartbeats the current one to the cloud middleman so the app always knows where to reach the Pi. Since plugin **0.6.23** the plugin's **tunnel watchdog** (above) restarts this service by itself if the tunnel goes unreachable from the internet while the Pi's own connection is fine, so a silently-dropped tunnel heals within ~15 minutes instead of leaving the printer offline remotely.

On a **LAN-only install** (answer **2) Direct (LAN/VPN)** at the installer's connection-mode question, or `install.sh --lan-only`; v0.9.51) neither the tunnel nor the auth proxy exists: the installer skips cloudflared entirely (including its architecture check), never creates the two systemd units, skips the clock check and its htpdate timer, and leaves Moonraker bound to the LAN. Run on a box that already has the tunnel stack, the same choice **retires** it - units disabled, the stale tunnel-URL logs removed so the plugin can't heartbeat a dead URL. The question only appears on interactive runs and defaults to the box's current mode; headless runs and explicit `MOONGATE_LAN_ONLY=` values never prompt. Cloud installs have the mirror-image knob: by default they bind Moonraker to localhost so only the token-gated proxy is network-exposed, and `MOONGATE_KEEP_BIND=1` keeps the existing bind instead - for setups where other machines must go on reaching Moonraker directly at `<pi-ip>:7125`, such as a central "farm" Mainsail (see [TROUBLESHOOTING.md](TROUBLESHOOTING.md)).
...
v0.9.49 closes the same gap on the **Pi side**: an orphaned plugin (its cloud record gone for good) used to heartbeat every 5 minutes forever. Plugin 0.6.15's heartbeat loop now goes **dormant** - a 6-hourly pulse - after a day of confirmed "no such printer" answers, or immediately on the explicit "released" reply that removing a printer now produces (see the tombstone note above). Any pairing activity wakes it instantly - and (0.6.19) an **open pairing session holds it awake**: a "released" reply is expected mid-re-pair (the replacement record only appears once the code is redeemed), so while an un-expired code is outstanding the loop keeps its fast cadence instead of dorming, and the freshly paired printer reports in within seconds.
